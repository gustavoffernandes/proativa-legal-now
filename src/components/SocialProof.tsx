      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="mt-10 sm:hidden -mx-4">
          <div className="snap-row">
            {quotes.map((q, i) => (
              <FadeInView key={q.name} delay={i * 0.1} className="snap-item">
                <figure className="rounded-2xl border border-border bg-card p-6 h-full">
        ...
        {/* Quotes — desktop grid */}
        <FadeInStagger className="mt-10 hidden sm:grid gap-4 sm:grid-cols-2" staggerDelay={0.12}>
          {quotes.map((q) => (
            <FadeInItem key={q.name}>
              <figure className="rounded-2xl border border-border bg-card p-6 sm:p-8 h-full">
